...
$this->values = $_POST; }else{
            $this->values = $values;
        }
}
public function input($type, $attributes = array(), $isMultiple = false) { $attributes['type'] = $type;
...
public function textarea($attributes = array()) { $name = $attributes['name'] ?? null;
...
return implode(' ', $tmp); }
protected function options($name, $options) { $tmp = array();
